...
def test_collection_saves_full_job_and_rejects_short_posting(tmp_path):
    session = Mock()
    session.get.side_effect = [
        FakeResponse(
            json.loads(
                (FIXTURES / "lever_page_1.json").read_text(encoding="utf-8")
            )
        ),
        FakeResponse(
            json.loads(
                (FIXTURES / "lever_page_2.json").read_text(encoding="utf-8")
            )
        ),
    ]
    source = LeverSource(
